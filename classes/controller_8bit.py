...
class Controller_Loop():
    def __init__(self):
        ...
        self.latest_state = {
            "pos": self.Robot.get_pos_xyz()[0:3],
            "jog": self.Robot.get_last_action(),
            "go": False,
            "gc": False,
            "frame": 0
        }
        ...
        print(f"Verbunden mit: {self.joystick.get_name()}")
    ...
    def Controller_Loop_run(self):
            print("Warte auf Eingaben... (Strg+C zum Beenden)")
            self.Sound.play_sound_start()
            while True:
                try:
                    # Joystick-Achsen
                        ...
                        if event.type == pygame.JOYBUTTONDOWN:
                            ...
                            if event.button == 1:
                                ...
                                if self.log_camera:
                                    try:
                                        camera_robot = camera_class(
                                            serial_camera=self.serial_robot,
                                            camera_name="robot",
                                        ...
                                        camera_context = camera_class(
                                            serial_camera=self.serial_context,
                                            camera_name="context",
                                            save_path=self.log_path,
                                            number_of_frame=0,
                                            depth_active=False,
                    ...
                    if self.log_camera and camera_robot is not None and camera_context is not None:
                        self.latest_state["pos"] = self.Robot.get_pos_xyz()[0:3]
                        self.latest_state["jog"] = self.Robot.get_last_action()
                        self.latest_state["go"] = open_gripper
                        self.latest_state["gc"] = close_gripper
                        ...
                        self.async_capture_once(camera_robot, state_copy, thread_name="robot_cam_once")
                        self.async_capture_once(camera_context, state_copy, thread_name="context_cam_once")
                        time.sleep(0.05)
